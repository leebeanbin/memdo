...
59 cases across 9 files, establishing what the current Agent (before Sprint 1's
...
- `expectedBehavior` is one of: `SEARCH_SCHEDULES`, `FIND_FREE_SLOTS`,
  `PROPOSE_SCHEDULE`, `PROPOSE_SCHEDULE_BATCH` (A3-1/A3-5 -- multiple NEW
  items staged from one call, e.g. "금요일 미용실 넣고 토요일 AWS 2시간,
  일요일 운동 넣어줘"; no search-before-propose requirement, same as
  `PROPOSE_SCHEDULE` -- a batch of new items has nothing existing to search
  for. `expected` stays `{}` for these cases: per-item fields live inside
  `items`, an array `grade.ts`'s `argsMismatches` only checks at the top
  level of `expected`, so batch fixtures are checked via manual review
  instead of pinned values), `PROPOSE_SCHEDULE_UPDATE`,
...
by default, which a single model run against this 59-case corpus already
