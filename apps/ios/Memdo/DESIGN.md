...
- 내 Agent 연결: Google Calendar·Slack MCP 도구, Memdo Agent 데이터 범위, 개인정보·보관 상태
- 메인 설정은 현재 값만 보여주고 연결 설명·AI 동의·개인정보·브리핑 키워드 편집은 행을 눌러 시트에서 연다.
- 브리핑 키워드 시트는 Reddit 관심사 선택처럼 직접 입력하거나 제안 키워드 중 최대 5개를 선택한다.
...
전체 달력 배경화면은 WidgetKit 위젯이 아니다. Memdo의 달력 이미지 생성 App Intent 결과를 단축어의 `배경화면 사진 설정` 액션에 전달하고, 사용자가 개인 자동화 시간을 선택하는 흐름으로 구현한다. 이미지 생성 액션이 준비되기 전에는 앱 안에서 디자인 시안만 제공한다.
