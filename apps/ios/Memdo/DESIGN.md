...
- 콜드 런치는 중앙 심벌만 최소 0.7초 보여준다. 인증 확인이 끝나지 않았으면 같은 화면을 유지하고, Reduce Motion에서는 전환 애니메이션을 사용하지 않는다.
- 로그아웃 상태 화면은 중앙의 `심벌 → 워드마크`와 하단의 Google·GitHub 로그인 버튼만 보여준다. 제품 설명, 글래스 패널, 권한 안내는 기본 화면에 두지 않는다.
