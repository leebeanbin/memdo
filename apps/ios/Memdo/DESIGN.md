...
- 기사 행을 누르면 상세 화면에서 분야·헤드라인·출처·핵심 내용을 확인한다. 원문 이동은 상세 화면 내비게이션 바의 보조 행동으로 둔다(별도 primary 버튼을 화면에 두지 않는다).
- 상세 화면은 분야를 이미 배지로 보여주므로 별도의 "주제 탐색" 화면으로 이동시키지 않는다. 관심사 추가·제거는 분야 배지 옆 토글로 그 자리에서 즉시 반영한다.
