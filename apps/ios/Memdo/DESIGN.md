...
- 화면은 중앙의 `심벌 → 워드마크 → 제품 가치`를 하나의 브랜드 히어로로 묶고, 하단에 `소셜 로그인 → 권한 안내`를 둔다.
- 중앙 심벌에는 별도 카드나 타일을 만들지 않는다. 글래스 표면은 하단 로그인 패널 하나만 사용한다.
- Google과 GitHub 버튼은 같은 52pt 높이와 정렬을 쓰되, 각 공급자의 공식 마크와 브랜드 규칙을 우선한다.
- 계정 없는 진입은 제공하지 않는다. 로그인 화면의 선택지는 Google과 GitHub로 제한한다.
- 로그인 화면도 다른 페이지와 같은 18pt 좌우 그리드를 사용한다.
- 소셜 로그인 버튼과 권한 안내는 하나의 26pt 글래스 패널에 묶는다. 외곽 10pt inset에 맞춰 내부 버튼 반경은 16pt를 사용하며 별도 카드를 추가하지 않는다.
...
- Google·GitHub가 기본 계정 인증에 쓰이므로 App Store 배포 전에는 심사 지침 4.8을 충족하는 동등한 개인정보 보호 로그인 수단을 추가하고 실제 인증까지 검증한다.
