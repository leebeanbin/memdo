...
enum AgentDateExpression: Equatable {
    case today, tomorrow, explicit(Date)

    /// nil means the model produced a token that isn't "today", "tomorrow",
    /// or a real yyyy-MM-dd calendar date -- callers must treat that as an
    /// explicit failure (reject the call, don't stage anything) rather than
    /// falling back to "today". `DateFormatting.posix`'s `DateFormatter` has
    /// `isLenient == false` by default, so an impossible date like
    /// "2026-02-31" already returns nil here without any extra configuration.
    init?(token: String) {
        switch token {
