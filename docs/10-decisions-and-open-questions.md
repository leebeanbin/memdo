...
| ADR-008 | MVP에는 자체 MCP를 만들지 않음 | 내부 함수 API로 충분 |
| ADR-009 | 최소 iOS 17 | 인터랙티브 위젯 단순화 |
| ADR-010 | 초기 작업 큐는 PostgreSQL integration_jobs | 50명 규모에 별도 브로커 불필요 |
...
| ADR-030 | 계정 없이 로컬 일정 사용, cloud 기능 활성화 시 Sign in with Apple | 첫 사용 마찰을 줄이면서 서버 데이터 소유자를 명확히 함 |
...
| ADR-036 | MVP 기본 AI 모델은 `gpt-5.6-luna`, 환경 변수 한 곳에서만 교체 | 비용 우선 파일럿에 맞추고 모델 교체 범위를 제한 |
...
| ADR-047 | MVP RRULE은 DAILY·WEEKLY와 선택 요일만 허용 | 반복 규칙 구현과 예외 처리를 제한 |
