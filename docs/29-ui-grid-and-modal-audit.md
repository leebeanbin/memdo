...
- 캘린더 검색은 라벨형 Glass 진입 버튼, Glass 검색 필드, 네이티브 segmented 범위 선택으로 이어진다.
...
- iOS 26에서는 interactive 조작층에 네이티브 `glassEffect`를 적용하고, 이전 버전은 material과 outline으로 대체한다.
...
| 검색 | 캘린더 검색 | 라벨형 Glass 진입·검색 필드·segmented 범위가 페이지 축 유지, 닫기 제공 |
